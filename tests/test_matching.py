...
def test_matching():
    ...
    # The BUY response was created before the SELL arrived, so it was OPEN then.
    # Fetch it again after matching to verify its updated state.
    updated_buy = service.get_order(buy["id"])

    assert updated_buy["status"] == "PARTIALLY_FILLED"
    assert sell["status"] == "FILLED"
    assert updated_buy["remaining_quantity"] == 1
